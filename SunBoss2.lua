...
SunBoss2.__index = SunBoss2

setmetatable(SunBoss2, {
	__index = Enemy,
	__call = function (cls, ... )
...
function SunBoss2:getType()
	return self.type
end

function SunBoss2:getHitBoxes( ... )
	local hb = {}
...
--returns the distance between SunBoss2' origin and (x, y)
function SunBoss2:distanceFrom(x, y)
	return math.sqrt((x - self.x)^2 + (y - self.y)^2)
end

function SunBoss2:hit()
...
function SunBoss2:alive()
	return self.health > 0
end

function SunBoss2:getHealth(...)
	return self.health
end

function SunBoss2:shoot()
	local b = EnemyBullet(self.x, self.y+40, 600, self.angle)
	table.insert(objects, b)
	self.s_timer = 0
end

function SunBoss2:collide(obj)
	if obj:getID() == 3 then
		self:hit()
		if not self:alive() then
			time = 0
